...
/**
 * Коридор под сегмент: сегментная карточка считает свой срез независимо от
 * фильтра по сегменту, поэтому и коридор берёт по своему коду, а не по фильтру.
 */
export function planForSegment(plans, { channel, segment, from, to, filters } = {}) {
  return planFor(plans, { channel, segment, from, to, filters });
}
