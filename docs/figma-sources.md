...
Node ID этого фрейма **не установлен**: чтобы его получить, нужен либо
`get_metadata` по доске целиком (ответ огромный), либо ссылка на фрейм от
заказчика. До этого сверяться по экспорту
